...
use solana_sdk::{
    address_lookup_table::AddressLookupTableAccount, compute_budget::ComputeBudgetInstruction, message::VersionedMessage, signature::{Keypair, Signer}, system_instruction, transaction::{Transaction, VersionedTransaction}
};
...
use crate::{routing_types::Route, server::{client_provider::ClientProvider, hash_provider::HashProvider}, swap::Swap};
use super::ix_sender::IxSender;
use once_cell::sync::Lazy;
use base64::{Engine as _, engine::general_purpose};

static NATIVE_MINT: Lazy<Pubkey> = Lazy::new(|| {
...
impl<THashProvider: HashProvider + Send + Sync + 'static> IxSender for JitoIxSender<THashProvider> {
    ...
    async fn  instructuin_extend(&self, swap: Arc<Swap>,route:Arc<Route>) -> anyhow::Result<HashMap<String, Vec<VersionedTransaction>>> {
        ...
        let new_profit = route.in_amount - route.out_amount;
        let compute_unit_limit = swap.cu_estimate + 20000;

        let jito_tip = self.calculate_tip(new_profit, compute_unit_limit);
        ...
        let tx2 = VersionedTransaction::try_new(tx2_message, &[&self.keypair])?;
    ...
    async fn send_tx(&self, transactions: HashMap<String, Vec<VersionedTransaction>>) -> anyhow::Result<()> {
            self.send_transaction_to_jito(transactions);
            Ok(())
    }
...
impl<THashProvider: HashProvider + Send + Sync + 'static> JitoIxSender<THashProvider> {
    pub fn new(
        ...
        let source_ata =
            get_associated_token_address(&keypair.pubkey(), &NATIVE_MINT);
        let public_key = keypair.pubkey();
        let jito_urls = jito_regions
            .iter()
            .map(|region| format!("https://{}.mainnet.block-engine.jito.wtf/api/v1/bundles", region))
            .collect::<Vec<_>>();
    ...
    async fn send_transaction_to_jito(&self, transactions: HashMap<String, Vec<VersionedTransaction>>)-> anyhow::Result<()> {
                            ...
                            Ok(response) => {
                                ...
                                let status = response.status();
                                        ...
                                        let bundle_uuid = body["result"]
                                            .as_str()
                                            .ok_or_else(|| anyhow!("Failed to get bundle UUID from response")).unwrap();
                                        info!("Response status: {} || Bundle sent with UUID: {}", status,bundle_uuid);
                            }
                            Err(e) => {
                                // let mut send_counter = self.send_counter.write().unwrap();
                                // let duration = send_start.elapsed();
                                ...
                                if let Some(status) = e.status() {
                                    match status.as_u16() {
                                        429 => {
                                            // 处理429错误
                                            error!("Received 429 Too Many Requests from Jito: {}", e);
                                        }
                                        400 => {
                                            // 处理400错误
